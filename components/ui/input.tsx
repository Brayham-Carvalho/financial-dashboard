...
function Input({ className, type, ...props }: React.ComponentProps<'input'>) {
  return (
    <input
      ...
      data-slot="input"
      className={cn(
        'file:text-foreground placeholder:text-muted-foreground/70 selection:bg-primary selection:text-primary-foreground h-11 w-full min-w-0 rounded-full border border-white/60 bg-white/70 px-4 text-sm shadow-[0_12px_32px_-24px_rgba(31,27,26,0.45)] outline-none transition-all duration-200 file:inline-flex file:h-8 file:rounded-full file:border-0 file:bg-primary/10 file:px-3 file:text-xs file:font-semibold disabled:pointer-events-none disabled:cursor-not-allowed disabled:opacity-50 dark:border-white/10 dark:bg-card/70',
